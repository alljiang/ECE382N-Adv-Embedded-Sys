...
#define GPIO_MAJOR 241                  // Need to mknod /dev/gpio_int c 243 0
#define MODULE_NM "gpio-interrupt"
...
static const struct of_device_id zynq_gpio_of_match[] = {
    { .compatible = "xlnx,int-latency-1.0" },
    { /* end of table */ }
};    
...
static int __init init_gpio_int(void)
{
    ...
    proc_gpio_int = proc_create("gpio-interrupt", 0444, NULL, &proc_fops );
...
no_gpio_interrupt:
    ...
    platform_driver_unregister(&zynq_gpio_driver);
    remove_proc_entry("gpio-interrupt", NULL);
    return -EBUSY;
};
...
static void __exit cleanup_gpio_interrupt(void)
{
    ...
    remove_proc_entry("gpio-interrupt", NULL);      // Remove process entry
    kfree(msg);
...
MODULE_DESCRIPTION("gpio-interrupt proc module");
MODULE_LICENSE("GPL");
